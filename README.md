...
(green — a change was applied), `⚠ Update available` (yellow — attention),
`ℹ Pinned` (cyan — not auto-updated), `✗ Error` / `✗ Update failed` (red).
"Updated" is used only when something actually changed; a bare "up to date"
is avoided because it reads like an action. Colors reinforce the words and
are enabled only on a terminal (`NO_COLOR` or `TERM=dumb` disables them);
piped and `--json` output never contains escape codes, and the symbols +
words carry the meaning in monochrome. The summary is one line per run
(`checked 4, updated 0, failed 0, held 0, pinned 0, errors 0`), colored
green / yellow / red by the worst outcome, plus a note when
`policy = "notify"` held updates (reported, never installed).
